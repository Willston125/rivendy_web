...
import { Utensils, Bike, BadgeCheck, Timer, Flame, ArrowRight } from "lucide-react";
import {
  isRestaurantOpen,
 ...
 *
 * Statut Ouvert/Fermé calculé côté serveur (heure serveur) à partir de
 * `horaires_ouverture` — approximation acceptable pour le SSR.
 */
  ...
  const inner = (
    <article className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm transition hover:shadow-md">
      ...
      <div className="relative h-32 w-full bg-[#E0F2F1]">
        ...
        {hasHours && (
          <span
            className={`absolute left-2.5 top-2.5 inline-flex items-center gap-1.5 rounded-full bg-white px-2.5 py-1 text-[11px] font-bold shadow-sm ${
        ...
        {group.hasPromo && (
          <span className="absolute right-2.5 top-2.5 inline-flex items-center gap-1 rounded-full bg-[#FF6B35] px-2.5 py-1 text-[11px] font-bold text-white shadow-sm">
